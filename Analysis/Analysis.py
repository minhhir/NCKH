...
def run_analysis(df):
    ...
    try:
        ...
        if p_h1 < 0.05 and b_h1 < 0:
            res_h1 = "Ủng hộ"
        elif p_h1 < 0.1 and b_h1 < 0:
            res_h1 = "Ủng hộ ở mức biên (có ý nghĩa xu hướng)"
        else:
            res_h1 = "Không ủng hộ"
    ...
    # Biến lưu trữ P-value để kiểm định chuỗi trung gian H9
    p_subj_trust = 1.0
    p_trust_phuman = 1.0
    ...
    try:
        ...
        results.append(f"- [H4] Tính chủ quan -> Niềm tin: β = {b_subj:.3f}, p = {p_subj:.3f} => {res_h4}")

        b_rl = t_b.get('Risk:Lit', 0);
        p_rl = t_p.get('Risk:Lit', 1)
        res_h5 = "Ủng hộ" if p_rl < 0.05 else "Không ủng hộ"
        results.append(f"- [H5] Rủi ro x Am hiểu AI -> Niềm tin: β = {b_rl:.3f}, p = {p_rl:.3f} => {res_h5}")

        b_sl = t_b.get('Subj:Lit', 0);
        p_sl = t_p.get('Subj:Lit', 1)
        res_h7 = "Ủng hộ" if p_sl < 0.05 else "Không ủng hộ"
        results.append(f"- [H7] Chủ quan x Am hiểu AI -> Niềm tin: β = {b_sl:.3f}, p = {p_sl:.3f} => {res_h7}")

        b_rs_t = t_b.get('Risk:Subj', 0);
        p_rs_t = t_p.get('Risk:Subj', 1)
        res_h10 = "Ủng hộ" if (p_rs_t < 0.05 and b_rs_t < 0) else "Không ủng hộ"
        results.append(f"- [H10] Rủi ro x Chủ quan -> Niềm tin: β = {b_rs_t:.3f}, p = {p_rs_t:.3f} => {res_h10}\n")
    ...
    formula_phuman = "P_human ~ Risk + Subj + Info + Lit + Trust_Norm + Risk:Lit + Risk:Subj + Risk:Info"
    try:
        ...
        results.append(f"- [H2] Niềm tin -> Hành vi: β = {b_trust:.3f}, p = {p_trust:.3f} => {res_h2}")

        b_rs_w = w_b.get('Risk:Subj', 0);
        p_rs_w = w_p.get('Risk:Subj', 1)
        res_h3 = "Ủng hộ" if (p_rs_w < 0.05 and b_rs_w > 0) else "Không ủng hộ"
        results.append(f"- [H3] Rủi ro x Chủ quan -> Hành vi: β = {b_rs_w:.3f}, p = {p_rs_w:.3f} => {res_h3}")

        b_info = w_b.get('Info', 0);
        p_info = w_p.get('Info', 1)
        res_h6 = "Ủng hộ" if (p_info < 0.05 and b_info < 0) else "Không ủng hộ"
        results.append(f"- [H6] Tải lượng thông tin -> Hành vi: β = {b_info:.3f}, p = {p_info:.3f} => {res_h6}")
        ...
        if p_ri < 0.05 and b_ri < 0:
            ...
            res_h8 = "Không ủng hộ (có ý nghĩa thống kê nhưng bị ngược chiều dự đoán)"
        else:
            res_h8 = "Không ủng hộ"
        results.append(f"- [H8] Rủi ro x Thông tin -> Hành vi: β = {b_ri:.3f}, p = {p_ri:.3f} => {res_h8}")

        res_h9 = "Ủng hộ" if (p_subj_trust < 0.05 and p_trust_phuman < 0.05) else "Không ủng hộ"
        results.append(f"- [H9] Biến trung gian (Niềm tin): Dựa trên chuỗi (Subj -> Trust -> P_human) => {res_h9}\n")

    except Exception as e:
        results.append(f"Lỗi mô hình Logit (Hành vi): {e}\n")

    # In ra terminal và lưu file
    output_text = "\n".join(results)
    print(output_text)

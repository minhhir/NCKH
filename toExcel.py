...
# raw data
raw_text = """
I. OBJECTIVE - LOW RISK (Khách quan - Rủi ro thấp)
(Tối ưu hóa, Tính toán chi phí, Hậu quả thấp)
...
II. SUBJECTIVE - LOW RISK (Chủ quan - Rủi ro thấp)
(Xã hội, Cảm xúc, Nhân sự - Hậu quả thấp)
...
III. OBJECTIVE - HIGH RISK (Khách quan - Rủi ro cao)
(Kỹ thuật, Sống còn, Tài chính lớn - Hậu quả nghiêm trọng)
...
IV. SUBJECTIVE - HIGH RISK (Chủ quan - Rủi ro cao)
(Đạo đức, Chiến lược, Nhân văn - Hậu quả nghiêm trọng)
...
def parse_questions_to_excel(text):
    # regex logic
    # 1. "Tình huống:" -> until "Lời khuyên AI"
    # 2. "Lời khuyên AI:" -> until"Lời khuyên con người"
    # 3. "Lời khuyên con người:" -> until "Tình huống" next OR title I, II, A, B OR to the end of text
    # use (?=...) (Lookahead) to stop matching at the next "Tình huống:" or "I.", "II.", "III.", "IV.", "A.", "B." or end of text

    pattern = r"Tình huống:\s*(.*?)\s*Lời khuyên AI:\s*(.*?)\s*Lời khuyên con người:\s*(.*?)(?=\n\s*(?:Tình huống:|I\.|II\.|III\.|IV\.|[A-Z]\.)|$)"

    # re.DOTALL use to make '.' match newline characters, allowing us to capture multi-line advice sections correctly
    ...
    output_file = "QuestionForm_cleantext.xlsx"
    df.to_excel(output_file, index=False)
    print(f"done-> {len(data)} question. Check file '{output_file}'")
    return df
